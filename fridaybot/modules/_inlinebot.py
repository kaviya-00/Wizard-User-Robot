...
if PMPERMIT_PIC is None:
    WARN_PIC = "https://telegra.ph/file/ec9265ba6a41c2a48c572.png"
else:
    WARN_PIC = PMPERMIT_PIC
LOG_CHAT = Config.PRIVATE_GROUP_ID
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "@Kaveesha_Induwara"
...
async def inline_handler(event):
    ...
    if event.query.user_id == bot.uid and query.startswith("Friday"):
        ...
        result = builder.article(
            title="Stats",
            text=f"**Showing Stats For {DEFAULTUSER}'s KaveeshaInduwara** \nNote --> Only Owner Can Check This \n(C) @Kaveesha_Induwara",
            buttons=[
                [custom.Button.inline("Show Stats ?", data="terminator")],
                [Button.url("Owner", "https://github.com/kaviya-admin")],
                [Button.url("Conatact ❤️", "t.me/Kaveesha_Induwara")],
            ],
        )
        await event.answer([result])
...
async def inline_handler(event):
    ...
    if query == None:
        resulte = builder.article(
            title="Usage Guide.",
            description="(C) @Kaveesha_Induwara",
